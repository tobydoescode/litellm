...
`requirements.in` holds the top-level pins (litellm, prisma). `requirements.txt`
is the fully resolved lockfile generated by `uv pip compile` — the Dockerfile
installs from that so builds are reproducible. Regenerate with `task lock`
